...
# Enhanced script to upload and apply USG config snippet via SSH with error handling
...
if ($UploadOutput -notmatch "usg-config.cfg" -or (Invoke-SSHCommand -SessionId $Session.SessionId -Command "wc -l < /tmp/usg-config.cfg").Output.Trim() -ne ($ConfigLines.Count).ToString()) {
    Write-Error "Upload failed or size mismatch. Output: $UploadOutput"
