...
if ($UploadOutput -notmatch "usg-config.cfg" -or $LineCountOutput -ne $ConfigLines.Count.ToString()) {
    Write-Error "Upload failed or size mismatch (expected $($ConfigLines.Count) lines, got $LineCountOutput). Output: $($UploadOutput | Out-String)"
